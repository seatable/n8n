...
export function ctx(api: IApi): ICtx {
		...
		if (['https://', 'https://'].includes(prepApi.server)) {
				throw new Error(`Expectation Failed: SeaTable: Context: Need server to create context, got URL-scheme only server "${prepApi.server}".`);
		}
